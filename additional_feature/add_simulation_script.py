# A forest-fire model following the principles of a cellular automaton model, with an additional model feature 
# Student ID: 1930864
...
def update_state(previous_grid):
    ...
    # Rule 0: Probability chance of a rain event, if true then all fires in the gird are put out
    if np.random.random() < r:
        new_grid[previous_grid == 2] = 1
...
p = 0.1
# The probability of rain putting out aLL fire (2 --> 1)
r = 0.05
...
# Saving the numpy grid to file "add_forest_simulation.npz"
np.savez_compressed("add_forest_simulation", state=grid)